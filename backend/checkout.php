...
if(isset($_POST['order_btn'])){
   ...
   if(!$number || !$email) {
      ...
      // Build address with sanitized inputs
      $address = mysqli_real_escape_string($conn, 'flat no. '. 
         filter_var($_POST['flat'], FILTER_SANITIZE_STRING) .', '. 
      ...
      if($cart_total == 0){
         ...
         $stmt = mysqli_prepare($conn, "SELECT * FROM `orders` WHERE name = ? AND number = ? AND email = ? AND method = ? AND address = ? AND total_products = ? AND total_price = ?");
         mysqli_stmt_bind_param($stmt, "sissssd", $name, $number, $email, $method, $address, $total_products, $cart_total);
         mysqli_stmt_execute($stmt);
         ...
         if(mysqli_num_rows($result) > 0){
            ...
            // Insert new order
            $stmt = mysqli_prepare($conn, "INSERT INTO `orders`(user_id, name, number, email, method, address, total_products, total_price, placed_on) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "isssssdss", $user_id, $name, $number, $email, $method, $address, $total_products, $cart_total, $placed_on);
